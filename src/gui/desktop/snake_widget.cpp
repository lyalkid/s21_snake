...
void Snake_Widget::paintEvent(QPaintEvent *e) {
  ...
  // фон поля
  p.fillRect(QRect(0, 0, WIDTH * size, HEIGHT * size), Qt::black);
  ...
  for (int i = 0; i < HEIGHT; ++i) {
    for (int j = 0; j < WIDTH; ++j) {
      int val = field[i][j];
      QRect cell(j * size, i * size, size, size);

      if (val == SNAKE) {
        p.fillRect(cell, Qt::green);
      } else if (val == HEAD) {
        p.fillRect(cell, Qt::yellow);
      } else if (val == BERRY) {
        p.fillRect(cell, Qt::red);
      }
    }
  }
  ...
  // --- боковая панель ---
  const int sideX = WIDTH * size + 10;
  QRect panel(sideX, 10, 120, 140);
  ...
  p.setFont(f);
  p.setPen(Qt::black);
  p.drawText(panel.adjusted(10, 10, -10, -90), Qt::AlignLeft,
             QString("Score: %1").arg(inf.score));
  p.drawText(panel.adjusted(10, 40, -10, -60), Qt::AlignLeft,
             QString("High: %1").arg(inf.high_score));
  p.drawText(panel.adjusted(10, 70, -10, -30), Qt::AlignLeft,
             QString("Level: %1").arg(inf.level));
}
